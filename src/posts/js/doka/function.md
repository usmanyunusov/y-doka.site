...
Это пример функции рекурсивной функции.

Рекурсией называется такая конструкция, при которой функция вызывает саму себя.

Достаточно часто такое применяется в математических операциях. Но не ограничивается ими.

Пример: при выводе объекта в как текст — он превращается в `[object Object]` и увидеть его содержимое проблематично. Функция ниже позволяет решить эту проблему:

🤖Не стоит пугаться огромных размеров кода функции, она представлена как пример реального использования рекурсии. Хотя она может и кажется сложной, на самом деле это просто множество проверок условий и формирование строки вывода.

<p class="codepen" data-height="265" data-theme-id="light" data-default-tab="js,result" data-user="vindi-r" data-slug-hash="XGGPOj" style="height: 265px; box-sizing: border-box; display: flex; align-items: center; justify-content: center; border: 2px solid; margin: 1em 0; padding: 1em;" data-pen-title="function рекурсия">
  <span>See the Pen <a href="https://codepen.io/vindi-r/pen/XGGPOj">
  function рекурсия</a> by vindi-r (<a href="https://codepen.io/vindi-r">@vindi-r</a>)
...
Особенно это может быть важно в колбеках — когда мы вызываем какую-то функцию в ответ на событие:
