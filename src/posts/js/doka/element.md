...
Элемент — это кусочек HTML в [DOM дереве](). Браузер создает DOM для взаимодействия между JavaScript и HTML. Каждый HTML тег при этом превращается в элемент DOM. Ещё такие элементы называют _узлами._
