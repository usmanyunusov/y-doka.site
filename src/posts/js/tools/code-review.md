...
Обычно разработчик отправляет на ревью набор изменений, который решает определенную задачу — добавляет новую функциональность или исправляет ошибку. Чаще всего, такие изменения программист [делает в своей ветке](/js/tools/version-control), а перед слиянием в основную, запрашивает обзор своих изменений у коллег.
...
Очень важно при код-ревью сосредотачиваться на том, что именно делает этот код, смотреть на его расширяемость, читаемость и удобство сопровождения. А расставленные пробелы, точки с запятой и другие мелочи лучше оставить [статическому анализатору](/js/tools/static-analysis).
